...
function search($searchstr, $category, $start, $per_page, $type, $domain) {
    ...
    $mysql_table_prefix = '';
    $possible_to_find = 1;
	//echo "select domain_id from " . $mysql_table_prefix . "domains where domain = '$domain'";exit;
...
function get_search_results($query, $start, $category, $searchtype, $results, $domain) {
    ...
    $show_query_scores = show_query_scores;
    ...
    if ($rows > 0) {
        ...
        while ($i < $num_of_results && $i < $results_per_page) {
            ...
            foreach ($words['hilight'] as $change) {
                ...
                if (strlen($urlstripped) > 40) {
                    $urlstripped = substr($urlstripped, 0, 40) . "...";
                }
                $url2 = $urlstripped;

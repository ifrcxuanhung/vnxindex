...
class CI_Template {
    ...
    function set_master_template($filename) {
        if (file_exists('_/templates/' . $filename) or file_exists('_/templates/' . $filename . EXT)) {
            $this->master = $filename;
        } else {
            show_error('The filename provided does not exist in <strong>templates</strong>. Remember to include the extension if other than ".php"');
        }
    }
    ...
    function initialize($props) {
        ...
        if (isset($props['template'])
                && (file_exists('_/templates/' . $props['template']) or file_exists('_/templates/' . $props['template'] . EXT))) {
            $this->master = $props['template'];
        } else {
            // Master template must exist. Throw error.
            show_error('Either you have not provided a master template or the one provided does not exist in <strong>templates</strong>. Remember to include the extension if other than ".php"');
        }
    ...
    function write_view($region, $view, $data = NULL, $overwrite = FALSE) {
        ...
        if (count($args) > 1) {
            foreach ($args as $suggestion) {
                if (file_exists('_/templates/' . $suggestion . EXT) or file_exists('_/templates/' . $suggestion)) {

 ...
 *
 */
class Vfdb_ion_auth_model extends CI_Model {
    ...
    public function __construct() {
        ...
        $this->tables = $this->config->item('tables', 'vfdb_ion_auth');

        //initialize data
        $this->identity_column = $this->config->item('identity', 'vfdb_ion_auth');
        $this->store_salt = $this->config->item('store_salt', 'vfdb_ion_auth');
        $this->salt_length = $this->config->item('salt_length', 'vfdb_ion_auth');
        $this->join = $this->config->item('join', 'vfdb_ion_auth');


        //initialize hash method options (Bcrypt)
        $this->hash_method = $this->config->item('hash_method', 'vfdb_ion_auth');
        $this->default_rounds = $this->config->item('default_rounds', 'vfdb_ion_auth');
        $this->random_rounds = $this->config->item('random_rounds', 'vfdb_ion_auth');
        $this->min_rounds = $this->config->item('min_rounds', 'vfdb_ion_auth');
        $this->max_rounds = $this->config->item('max_rounds', 'vfdb_ion_auth');


        //initialize messages and error
        $this->messages = array();
        $this->errors = array();
        $this->message_start_delimiter = $this->config->item('message_start_delimiter', 'vfdb_ion_auth');
        $this->message_end_delimiter = $this->config->item('message_end_delimiter', 'vfdb_ion_auth');
        $this->error_start_delimiter = $this->config->item('error_start_delimiter', 'vfdb_ion_auth');
        $this->error_end_delimiter = $this->config->item('error_end_delimiter', 'vfdb_ion_auth');
    ...
    public function register($username, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        $manual_activation = $this->config->item('manual_activation', 'vfdb_ion_auth');
        ...
        $default_group = $this->where('name', $this->config->item('default_group', 'vfdb_ion_auth'))->group()->row();
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                if ($remember && $this->config->item('remember_users', 'vfdb_ion_auth')) {
    ...
    public function is_max_login_attempts_exceeded($identity) {
        if ($this->config->item('track_login_attempts', 'vfdb_ion_auth')) {
            $max_attempts = $this->config->item('maximum_login_attempts', 'vfdb_ion_auth');
            if ($max_attempts > 0) {
     ...
     * */
    public function update($id, array $data, $group_id) {
        ...
        $data_group = array('group_id' => $group_id);
        $this->db->update($this->tables['users_groups'], $data_group, array('user_id' => $user->id));
    ...
    public function set_lang($lang = 'en') {
        ...
        if ($this->config->item('user_expire', 'vfdb_ion_auth') === 0) {
            $expire = (60 * 60 * 24 * 365 * 2);
        }
        // otherwise use what is set
        else {
            $expire = $this->config->item('user_expire', 'vfdb_ion_auth');
        }
    ...
    public function login_remembered_user() {
        ...
        if ($query->num_rows() == 1) {
            ...
            if ($this->config->item('user_extend_on_login', 'vfdb_ion_auth')) {
                $this->remember_user($user->id);
            }
    ...
    public function get_users_roles($id = FALSE) {
        $id || $id = $this->session->userdata('user_id');
        return $this->db->select('role.role_id,name')
                        ->where('user_id', $id)

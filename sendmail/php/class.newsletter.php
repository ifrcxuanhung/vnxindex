...
/**
 * Simple Newsletter Class
 *
 * Author: David Baker
 * Email: dtbaker@gmail.com
 * Version 1.7
 * License: Single use only License after purchase from ThemeForest.
 * Using this for multiple clients requires multiple license purchses.
 *
 * Change log:
 * - Version 1: 	initial version
 * - Version 1 to 1.6: - whole bunch of new features.
 * - Version 1.7: - campaigns.
 */
class newsletter {
    ...
    public function french_dateformat($date) {
    ...
    public function get_template_html($template, $replace, $options = array()) {
        ...
        $template_file = _TEMPLATE_DIR . basename($template) . "/template.php";
    ...
    public function write_newsletter_content($db, $newsletter_id) {
        ...
        $newsletter_data = $this->get_newsletter($db, $newsletter_id);
        	//echo "<pre>";print_r($newsletter_data);exit;
        $email_top = getArticleByCodeCategory('email_top');
        $email_bottom = getArticleByCodeCategory('email_bottom');
        ...
        $replace = array(
			...
			"EMAIL_HEADER" => $email_top[0]['description'],
            "EMAIL_BOTTOM" => $email_bottom[0]['description'],
        );
    ...
    public function get_newsletter($db = false, $newsletter_id) {
       ...
       $link = PARENT_URL . 'assets/upload/images/';
	   
       $newsletter['content'] = str_replace('../assets/upload/images/', $link, $newsletter['content']);
    ...
    public function get_newsletters($db = false) {
        if (!$db
            ...
            $db = db_connect();
        $sql = "SELECT * FROM " . _DB_PREFIX . "newsletter ORDER BY create_date DESC";
        return qa($sql, $db);
    }
    ...
    public function save_group($db = false, $group_id = 'new', $group_name, $category_name, $public = 0, $type = 0) {
        if (strpos($group_name, $category_name) === FALSE) {
            ...
            if ($category_name != '')
                $group_name = trim($category_name) . ' > ' . $n;
            else
                $group_name = $n;
        }
        if (!$db
            ...
            $db = db_connect();
        if ($group_id == 'new') {
            $sql = "INSERT INTO `" . _DB_PREFIX . "group` SET group_name = '" . mysql_real_escape_string($group_name) . "', `public` = '" . (int) $public . "', category_name = '$category_name', `type` = '" . (int) $type . "'";
        } else {
            $sql = "UPDATE `" . _DB_PREFIX . "group` SET group_name = '" . mysql_real_escape_string($group_name) . "', `public` = '" . (int) $public . "', category_name = '$category_name', `type` = '" . (int) $type . "' WHERE group_id = '" . mysql_real_escape_string($group_id) . "'";
        }
        $res = query($sql, $db);
        if ($group_id == 'new') {
    ...
    public function get_image($db, $id) {
	 ...
	 public function getListMemberField($db) {
        $sql = "SELECT * FROM `" . _DB_PREFIX . "member_field`";
        return qa($sql, $db);
    }
    ...
    public function get_members($db, $group_id = false, $newest_first = false, $limit = false, $search = array(), $sort = '', $type = '') {
        if (!$db)
            $db = db_connect();
        $sql = "SELECT `m`.`member_id`, `m`.`email`, `m`.`last_name`, `m`.`m_city`
                FROM (
                    SELECT `" . _DB_PREFIX . "member`.*, `" . _DB_PREFIX . "member_field_value`.`value` AS `m_city`
                    FROM `" . _DB_PREFIX . "member`, `" . _DB_PREFIX . "member_field`, `" . _DB_PREFIX . "member_field_value`
                    WHERE `" . _DB_PREFIX . "member`.`member_id` = `" . _DB_PREFIX . "member_field_value`.`member_id`
                    AND `" . _DB_PREFIX . "member_field_value`.`member_field_id` = `" . _DB_PREFIX . "member_field`.`member_field_id`
                    AND `" . _DB_PREFIX . "member_field`.`field_name` = 'City') AS `m`";
        ...
        /* join to sort by city */
        if ($sort == 'city') {
            $sql .= " LEFT JOIN (SELECT mfv.member_id, mfv.`value` as `city`
                    FROM member_field mf, member_field_value mfv
                    WHERE mf.member_field_id = mfv.member_field_id AND mf.field_name = 'City') A
                    ON m.member_id = A.member_id ";
        }
        /* ------ end sort by city left join ------ */
        $sql .= " WHERE 1 ";
        ...
        if (isset($search['start-letter']) && $search['start-letter']) {
            if ($search['start-letter'] == '#') {
                /* pull all non-alpha starting last_name */
                if ($sort != '') {
                    $sql .= " AND {$sort} NOT RLIKE '^[a-zA-Z]' ";
                } else {
                    $sql .= " AND m.last_name NOT RLIKE '^[a-zA-Z]' ";
                }
            } else {
                /* serach letter here */
                if ($sort != '') {
                    $sql .= " AND {$sort} LIKE '" . mysql_real_escape_string($search['start-letter']) . "%' ";
                } else {
                    $sql .= " AND m.last_name LIKE '" . mysql_real_escape_string($search['start-letter']) . "%' ";
                }
            }
        }
	...
	public function getMemberField($db){
		 if (!$db)
        $db = db_connect();
		$sql_member_field = "SELECT field_name FROM member_field WHERE active = 1";
		$kq = qa($sql_member_field, $db);
		 foreach ($kq as $field) {
			$field_name .=  "{ label : '$field[field_name]',name: '$field[field_name]',width: 120},";
		 }	
		 return $field_name;
	}
	
	
	public function get_members_all($db, $group_id = false, $newest_first = false, $limit = false, $search = array(), $sort = '', $type = '') {
        if (!$db)
        $db = db_connect();
		$sql_member_field = "SELECT field_name FROM member_field WHERE active = 1";
		$kq = qa($sql_member_field, $db);
		 foreach ($kq as $field) {
			$field_name .=  "max(case when t2.`field_name` = '$field[field_name]' then t2.value end) `$field[field_name]`, ";
		 }
		
		
        $sql = "select t1.*, $field_name
            GROUP_CONCAT(DISTINCT t3.group_name
        ORDER BY t3.group_name ASC
        SEPARATOR ' + ') AS group_name
            
            from member t1
            left join (select mf.*, mfv.value, mfv.member_id FROM member_field as  mf JOIN member_field_value as mfv ON  mf.member_field_id=mfv.member_field_id ) t2
              on t1.member_id = t2.member_id
            left join (select gp.group_name, mb.member_id, mb.group_id FROM member_group as  mb JOIN `group` as gp ON  mb.group_id=gp.group_id) t3 
              on t3.member_id = t2.member_id
            group by t1.member_id, t1.email";
		
		//echo "<pre>";print_r($sql);exit;
			
			/* $sql = "select t1.*,
              max(case when t2.`field_name` = 'address' then t2.value end) address,
              max(case when t2.`field_name` = 'Zip code' then t2.value end) `Zip code`,
              max(case when t2.`field_name` = 'City' then t2.value end) City,
              max(case when t2.`field_name` = 'Phone' then t2.value end) Phone,
			  max(case when t2.`field_name` = 'Test' then t2.value end) TEST,
            GROUP_CONCAT(DISTINCT t3.group_name
        ORDER BY t3.group_name ASC
        SEPARATOR ' + ') AS group_name
    ...
    public function get_article_all($db, $group_id = false, $newest_first = false, $limit = false, $search = array(), $sort = '', $type = '') {
        if (!$db)
        $db = db_connect();
        $sort = ($sort == "") ? 'DESC' : 'ASC';
        ...
        $sql = "SELECT a.article_id, a.category_id, a.image, a.viewed, a.date_added, a.date_modified, a.url, ad.title, ad.description, ad.long_description, ad.meta_description, ad.meta_keyword, c.category_code 
                ...
                WHERE a.article_id = ad.article_id AND a.category_id = c.category_id AND ad.lang_code = 'us' AND a.status = 1 AND c.category_code = 'sendmail' AND a.sort_order>0
    ...
    public function get_members_or($db, $group_id = false, $newest_first = false, $limit = false, $search = array(), $sort = '', $type = '') {
        if (!$db)
            $db = db_connect();
        ...
        /* easy upgrade, loop it up in the where department: */
        foreach ($search['group_id'] as $search_group_id => $tf) {
            $sql .= " OR mg.group_id = '" . (int) $search_group_id . "' ";
            /* echo "Group $search_group_id ". $sql; */
        }
        if (isset($search['name']) && $search['name']) {
            $sql .= " AND (m.first_name LIKE '%" . mysql_real_escape_string($search['name']) . "%' OR m.last_name LIKE '%" . mysql_real_escape_string($search['name']) . "%')";
	...
	public function get_member_fields_hung($db) {
        if (!$this->custom_fields) {
            $sql = "SELECT *,member_field_id AS id FROM " . _DB_PREFIX . "member_field WHERE active = 1";
            $this->custom_fields = qa($sql, $db);
        }
        return $this->custom_fields;
    }

    public function save_member_fix($db = false, $member_id = 'new', $fields, $from_public_sub = false) {
		if (!$db
        )
            $db = db_connect();
        if (!count($fields)
            ...
            return;
        if ($fields['email'] != '' && !preg_match("/^([a-zA-Z0-9])+([a-zA-Z0-9\._-])*@([a-zA-Z0-9_-])+([a-zA-Z0-9\._-]+)+$/", $fields['email'])) {
            return false;
        }
		...
		*/
        ...
        if ($fields['email']) {
            ...
            $sql = "SELECT `" . _DB_PREFIX . "member`.*
					FROM `" . _DB_PREFIX . "member` 
					WHERE email = '" . mysql_real_escape_string($fields['email']) . "' LIMIT 1";
            ...
            if ($existing) {
				...
				$dt[1]=$address = $fields['custom']['Address'];
				$dt[2]=$zip_code = $fields['custom']['Zip code'];
				$dt[3]=$city = $fields['custom']['City'];
				$dt[4]=$phone = $fields['custom']['Phone'];
				$group_id = $fields['group_id'];
				
				for($i=1; $i<=4; $i++)
				{
					$sql = "Update `member_field_value` set `value` = '".$dt[$i]."' where `member_id` = $member_id and `member_field_id` = $i";
					$res = query($sql, $db);
				}
				
				$sql = "delete from member_group where `member_id` = $member_id";
				$res = query($sql, $db);
				for($i=0; $i<count($group_id); $i++)
				{
					$sql = "insert into member_group(member_id, group_id) values ($member_id, $group_id[$i])";
					$res = query($sql, $db);
				}
            }
        }
	...
	public function save_member($db = false, $member_id = 'new', $fields, $from_public_sub = false) {
        ...
        if ($fields['email']) {
            ...
            /* see if this member already exists */
            $sql = "SELECT `" . _DB_PREFIX . "member`.*
					FROM `" . _DB_PREFIX . "member`, `" . _DB_PREFIX . "member_group`
					WHERE email = '" . mysql_real_escape_string($fields['email']) . "'
					AND `" . _DB_PREFIX . "member`.`member_id` = `" . _DB_PREFIX . "member_group`.`member_id`
					AND `" . _DB_PREFIX . "member_group`.`group_id` = '{$groups}'";
            $existing = array_shift(qa($sql, $db));
            if ($existing) {
                $member_id = $existing['member_id'];
            }
	...
	public function delete_member($db, $member_id) {
        ...
        $sql = "DELETE FROM `" . _DB_PREFIX . "member_group` WHERE member_id = '" . mysql_real_escape_string($member_id) . "'";
        $res = query($sql, $db);
		$sql = "DELETE FROM `" . _DB_PREFIX . "member_field_value` WHERE member_id = '" . mysql_real_escape_string($member_id) . "'";
        $res = query($sql, $db);
    }
    
  //  public function delete_newsletters($db, $member_id) {
//        $sql = "DELETE FROM `" . _DB_PREFIX . "member` WHERE member_id = '" . mysql_real_escape_string($member_id) . "' LIMIT 1";
//        $res = query($sql, $db);
//        $sql = "DELETE FROM `" . _DB_PREFIX . "member_group` WHERE member_id = '" . mysql_real_escape_string($member_id) . "'";
//        $res = query($sql, $db);
...
//    }
    ...
    public function test_sync($db, $sync_id) {
        ...
        if ($sync['db_table_fname_key'])
            $sql .= ", `" . mysql_real_escape_string($sync['db_table_fname_key']) . "`";
        if ($sync['db_table_lname_key'])
            $sql .= ", `" . mysql_real_escape_string($sync['db_table_lname_key']) . "`";
                ...
                if ($sync_unique_id) {
                    ...
                    if ($email_address) {
                        ...
                        if ($member_id) {
                            ...
                            if ($sync['db_table_fname_key'])
                                $sql .= ", first_name = '" . mysql_real_escape_string($row[$sync['db_table_fname_key']]) . "'";
                            if ($sync['db_table_lname_key'])
                                $sql .= ", last_name = '" . mysql_real_escape_string($row[$sync['db_table_lname_key']]) . "'";
    ...
    public function get_form($db, $include_form_tag = false, $member_id = false, $admin = false) {
        ...
        <?php if ($include_form_tag) { ?>
            ...
            <input type="hidden" name="member_id" value="<?php echo ($member_id != "")? $member_id : "" ?>" />
            <?php } ?>
            <table class="table table-hover" style="clear: both;">
                <tbody>
                    <tr>
                        <td>Email <span class="required">*</span></td>
                        <td><input type="text" class="text_input" name="email" value="<?php echo htmlspecialchars($member_data['email']); ?>"></td>
                    </tr>
                    <tr>
                        <td>Civilité <span class="required">*</span></td>
                        <td><input type="text" class="text_input" name="civilite" value="<?php echo htmlspecialchars($member_data['civilite']); ?>"></td>
                    </tr>
                    <tr>
                        <td>First Name <span class="required">*</span></td>
                        <td><input type="text" class="text_input" name="first_name" value="<?php echo htmlspecialchars($member_data['first_name']); ?>"></td>
                    </tr>
                    <tr>
                        <td>Last Name</td>
                        <td><input type="text" class="text_input" name="last_name" value="<?php echo htmlspecialchars($member_data['last_name']); ?>"></td>
                    </tr>
                    <tr>
                        <td>Société </td>
                        <td><input type="text" class="text_input" name="societe" value="<?php echo htmlspecialchars($member_data['societe']); ?>"></td>
                    </tr>
                    <?php
                        ...
                        <td>
                            <table>
                                ...
                                foreach ($groups as $group) {
                                    ...
                                    ?>
                                    <tr>
                                        <td><input type="checkbox" name="group_id[]" value="<?php echo $group['group_id']; ?>" <?php
                        if (!$member_id || $member_data['groups'][$group['group_id']])
                            echo 'checked';
                                    ?>></td>
                                ...
                                if ($campaigns) {
                                    foreach ($campaigns as $campaign) {
                                        ...
                                        ?>
                                        <tr>
                                            <td><input type="checkbox" name="campaign_id[]" value="<?php echo $campaign['campaign_id']; ?>" <?php
                        if (!$member_id || $member_data['campaigns'][$campaign['campaign_id']])
                            echo ' checked';
                                        ?>></td>
	...
	public function edit_del_add($db,$table){
		$data = array();
		$where = "where 1=1";
		if($_POST['oper'] == 'del'){
			$sql = "delete from $table where member_field_id=$_POST[id]";
        	query($sql, $db);
			echo "true";
		}
		if($_POST['oper'] == 'add'){
			foreach($_POST as $k=>$v){
				if($k!= 'oper' && $k!= 'member_field_id')
					$data[$k] = $v;	
			}
		 ...
		 if($_POST['oper'] == 'edit'){
			...
			//echo "<pre>";print_r($data);exit;
			 $sql = "update $table set ";
			$keyend = end(array_keys($data));
			foreach ($data as $key => $va) {
				$sql .= "$key = '$va' ";
				if ($key != $keyend)
					$sql .= ", ";
			}
			$sql .= "where member_field_id = $_POST[member_field_id]";
			query($sql, $db);
			echo "true";			
		 }
	}

...
switch($_REQUEST['t']){
	case "signup":
		$message = false;
		switch(true){ default:
			// hacky switch so we can break out of it on error.
			$groups = array();
            ...
            $newsletter->save_member($db, $member_id, $fields);
			...
			foreach($existing_custom_fields as $key=>$val){
				if($val['required'] && !$custom[$val['field_name']]){
					$message = "Please go <a href='javascript:history.go(-1);'>back</a> and complete all required fields.";
					break;
				}
			}
			if($message)break;
	...
	case "update_form":
		...
		if(isset($_REQUEST['email'])){
			...
			if($existing){
				...
				$email = '';
				$message = '<p>Sorry, we could not find your email address in our database. Please check your email address and try again.</p>';
			}
		}
		if(!$member_id){
	...
	case "view":
		...
		if(($mhash_real == $mhash_provided) || ($provided_hash == $real_hash)){
			...
			$replace = array(
				...
				"sent_date" => date("d-m-Y"), // TODO - read newsletter date

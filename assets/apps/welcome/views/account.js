                ...
                if(action != "logout"){
                    if(action == "login"){
                        html = '<form id="login-form" method="post">'+
                            ...
                            '<label style="float: left; width: 90px; margin-left: 62px; color: #fff;">E-mail</label>'+
                            '<input type="text" name="identity" style="margin-bottom: 10px; width: 250px" /><br />'+
                            '<label style="float: left; width: 90px; margin-left: 62px; color: #fff;">Password</label>'+
                            ...
                            '<div style="margin-bottom: 10px; color: #fff;"><input type="checkbox" name="remember" style="margin-left: 0" />Remmember me</div>'+
                            '<label style="float: left; width: 90px; margin-left: 62px">&nbsp;</label>'+
                            '<div style="margin-bottom: 10px"><button type="submit" name="submit" class="ui-button">' + action + '</button></div>'+
                            '<a style="color:#fff" class="forgotten_password" href="#">Forgot password?</a></form>';
                        width = 500;
                        height = 'auto';
                    }
                    if(action == 'register'){
                                            ...
                                            if(rs.error != ''){
                                                ...
                                                if(action == "login"){
                                                    $.ajax({
                                                        ...
                                                        type: 'post',
                                                        data: $("#login-form").serialize() + "&act_from=fe",
                                                        async: false,

    ...
    var currencyListView = Backbone.View.extend({
            ...
            $.modal({
                ...
                buttons: {
                    'Yes': function(win) {
                        win.closeModal();
                        $("#lean_overlay").show();
                        var $menu = trans("mn_import_currency", 1);
                        var $start = new Date().getTime() / 1000;
                        $.ajax({
                            type: "POST",
                            url: $admin_url + "observatory/import_currency_day",
                            success: function() {
                                $.ajax({
                                    url: $admin_url + "observatory/import_currency_month",
                                    type: "POST",
                                    async: false
                                });
                                $.ajax({
                                    url: $admin_url + "observatory/import_currency_year",
                                    type: "POST",
                                    async: false
                                });
                                $("#lean_overlay").hide();
                                ...
                                openModal($menu, "<div class='import-currency-report'></div>", 400);
                                var datatemplate = {};
                                datatemplate.report = $time;
                                var compiledTemplate = _.template(reportTemplate, datatemplate);
                                $(".import-currency-report").html(compiledTemplate).fadeIn();
                            }
                        });
                    },
                    'Cancel': function(win) {
        ...
        index: function() {
            $(document).ready(function() {
                ...
                oTable = $('#vndb_currency_ref').dataTable({
                    "oLanguage": {
                        "sUrl": $file
                    },
                    "bRetrieve": true,
                    ...
                    sDom: '<"block-controls"<"export-buttons"><"controls-buttons"p><"my-buttons"><"filter"f>>rti',
        ...
        CompareCurrency: function(event) {
            ...
            {
                case 'day':
                    var $table = 'vndb_currency_day';
                    break;
                case 'month':
                    var $table = 'vndb_currency_month';
                    break;
                case 'year':
                    var $table = 'vndb_currency_year';
                    break;
                default:
                    var $table = 'vndb_currency_day';
                    break;
        ...
        chart_filter: function($table, $curr1, $curr2) {
            var chartHeight = 450;
            if ($table == 'vndb_currency_year') {
                chartHeight = 450;
                $('#tab-global').attr('style', 'width: 44.8%; float: left; position: relative; height: 410px !important; margin-right: 20px;');
            }
            $.ajax({

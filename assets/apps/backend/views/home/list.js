...
define([
    ...
    var homeListView = Backbone.View.extend({
        ...
        events: {
            ...
            "click .action-active-user": "activeUser",
            "click .action-active-article": "activeArticle"
        },
        activeUser: function(event) {
            var $this = $(event.currentTarget);
            var title = trans('mn_Confirmation', 1);
                    ...
                    'Yes': function() {
                        ...
                        $.ajax({
                            ...
                            success: function() {
                                $('#lean_overlay').hide();
                                $.modal({
                                    content: trans('update successful', 1),
                                    title: title,
                                    maxWidth: 2500,
                                    width: 400,
                                    buttons: {
            ...
            $.modal({
                ...
                buttons: {
                    'Yes': function() {
                        ...
                        var id = $($this).attr('value');
                        $.ajax({
                            type: "POST", url: $admin_url + "article/chang_active",
                            data: "id=" + id,
                            error: function(jqXHR, exception) {
                                ajaxError(jqXHR, exception);
                            },
            ...
            var getTable = function(current) {
                ...
                $.ajax({
                    ...
                    success: function(rs) {
                        ...
                        var myGrid = $($idRender).jqGrid({url: $url + '?table=' + $table + '&code=' + $code,
                            datatype: 'json',
                            mtype: 'GET',
        ...
        index: function() {
            ...
            $(document).ready(function() {
                ...
                var rm_list = ["block-footer"];
                $(rm_list).each(function(i) {
                    $("#tabs-users ." + rm_list[i]).remove();
                    $("#tabs-article ." + rm_list[i]).remove();
                });
            });
        },
        render: function() {

...
define([
    ...
    var sysformatListView = Backbone.View.extend({
        ...
        events: {
            ...
            "click a.action": "showTableAll", //showTable
            "click a.action-view-all": 'showTableAll',
        ...
        showTableAll: function(event) {
            ...
            openModal(title, content, $widthModal);
            if ($('#modal-table-view-list').length > 0) {
                sysformatListView.showAllFields('#modal-table-view-list', title, ($widthModal - 10));
            }
            return false;
        },
        index: function($idRender, $table, $width, $check) {
            var myGrid;
            if ($check === undefined) {
                $check = "";
            }
            ...
            $(document).ready(function()
            {
                    ...
                    $.ajax({
                        ...
                        success: function(rs) {
                            ...
                            if (rs != 0) {
                                ...
                                myGrid = $($idRender).jqGrid({
                                    ...
                                    loadComplete: function(response) {
                                        ...
                                        require(['views/home/list'], function(obj) {
                                            obj.fixPositionsOfFrozenDivs($this);
                                ...
                                $($idRender + "_frozen tr").on("dblclick", function() {
                                    $id = $(this).attr("id");
                ...
                getTable();
                if ($check) {
                    $($idRender).jqGrid('inlineNav', '#pager');
                }
                $($idRender).jqGrid(
                        'navButtonAdd',
                        '#pager',
        ...
        showAllFields: function($idRender, $table, $width, $check) {
            var myGrid;
            if ($check === undefined) {
                $check = "";
            }
            ...
            $(document).ready(function()
            {
                ...
                var getTable = function(current) {
                    ...
                    $.ajax({
                        url: $base_url + 'backend/sysformat/getConfigFull?table=' + $table,
                        type: 'post',
                        async: false,
                        error: function(jqXHR, exception) {
                            ajaxError(jqXHR, exception);
                        ...
                        success: function(rs) {
                            ...
                            if (rs != 0) {
                                ...
                                myGrid = $($idRender).jqGrid({
                                    ...
                                    width: $width,
                                    caption: trans("sysformat_" + $table)
                                }).jqGrid('navGrid', '#pager', {edit: false, add: false, del: true, csv: true});
